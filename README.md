...
* **智能导航**：自动从导航入口解析可用真实网址，多候选随机返回。
* **站内搜索**：支持搜书吧与 SXSY 关键词搜索。
* **直接下载**：支持将帖子序号作为参数直接发起附件下载。
* **自动补档**：文件发送失败时自动打包 ZIP 重发，支持配置加密密码。
...
* `/ssb <帖子URL>`：直接解析并下载该帖子附件。

> [!NOTE]
> `/ssb <关键词>` 需要配置 `账号密码`（格式：`账号&密码`）。
...
> [!NOTE]
> `/sxsy <关键词>` 需要配置 `网站地址` 与 `Cookie`（用于搜索和状态监控的站点地址）。
...
* `/uaa` 或 `/有爱爱`：获取有爱爱最新地址。
* `/jm` 或 `/禁漫天堂`：获取禁漫天堂最新地址和 APP 下载地址。

### 4. 网站状态监控（管理员）

* `/ssbmon` 或 `/监控搜书吧`：在当前会话订阅搜书吧状态通知。
* `/ssbmon off`：在当前会话取消搜书吧状态订阅。
* `/sxsymon` 或 `/监控SXSY`：在当前会话订阅 SXSY 状态通知。
* `/sxsymon off`：在当前会话取消 SXSY 状态订阅。
